...
class TransformCodec extends Codec {
    ...
    encodeObject(object, props, transform) {
        ...
        return Promise.map(Object.keys(props), prop => {
            ...
            return Promise.resolve(this.encodeValue(object[prop], props[prop], transform[prop]))
            .then(encoded => {
    ...
    _encodeArrayValue(value, Type, transform) {
        ...
        return Promise.map(value, item => {
            ...
            return Promise.resolve(this.encodeValue(item, Type[0], transform));
        });
    }
    decode(json) {
    ...
    decodeObject(json, props, transform) {
        ...
        return Promise.map(Object.keys(props), prop => {
            ...
            return Promise.resolve(this.decodeValue(json[alias], props[prop], transform[prop]))
            .then(decoded => {
                result[prop] = decoded;
            });
        })
    ...
    decodeValue(value, Type, transform) {
        ...
        switch (Type) {
            ...
            case Date:
                return this.normalize ? new Date(value.getTime()) : value;
            default:
    ...
    decodeArrayValue(value, Type, transform) {
        ...
        return Promise.map(value, item => {
            ...
            return Promise.resolve(this.decodeValue(item, Type[0], transform));
        });
    }
}
