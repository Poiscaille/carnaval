const PreCodec = require('./codecs/PreCodec');
const TransformCodec = require('./codecs/TransformCodec');

const Promise = require('./promise-polyfill');

class Mapper {
    constructor(Clazz) {
        this.hooks = {};
        this.helpers = {};

        this.preCodec = new PreCodec(Clazz, Mapper);
        this.transformCodec = new TransformCodec(Clazz, Mapper);
    }
    with(schema) {
        this.preCodec.setSchema(schema);
        this.transformCodec.setSchema(schema);
        return this;
    }
    defaults(options) {
        this.transformCodec.setPermissions(options.permissions);
        return this;
    }
    normalize(normalize) {
        this.transformCodec.setNormalize(normalize);
        return this;
    }
    providers(helpers) {
        this.helpers = helpers;
        this.transformCodec.setHelpers(helpers);
        return this;
    }
    beforeEncode(...fn) {
        this.hooks['before:encode'] = fn;
        return this;
    }
    beforeDecode(...fn) {
        this.hooks['before:decode'] = fn;
        return this;
    }
    afterEncode(...fn) {
        this.hooks['after:encode'] = fn;
        return this;
    }
    afterDecode(...fn) {
        this.hooks['after:decode'] = fn;
        return this;
    }
    _hook(event, value) {
        if (!this.hooks[event]) {
            return Promise.resolve(value);
        }
        return Promise.reduce(this.hooks[event], (memo, middleware) => {
            return Promise.resolve(middleware(memo, this.helpers))
            .then(result => result ? result : memo);
        }, value);
    }
    encode(object) {
        return this._hook('before:encode', this.preCodec.encode(object))
        .then(object => this.transformCodec.encode(object))
        .then(json => this._hook('after:encode', json));
    }
    decode(json) {
        return this._hook('before:decode', this.preCodec.decode(json))
        .then(json => this.transformCodec.decode(json))
        .then(object => this._hook('after:decode', object));
    }
}

module.exports = Mapper;
