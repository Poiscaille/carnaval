    ...
    _hook(event, value, original) {
        ...
        return Promise.reduce(this.hooks[event], (memo, middleware) => {
            if (Array.isArray(memo)) {
                return Promise.map(memo, (item, index) => Promise.resolve(middleware(item, original[index], this.helpers)).then(result => result ? result : item));
            }
