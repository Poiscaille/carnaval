...
class ClassCodec {
    ...
    encode(object, providers) {
        const result = {};
        return this.mapping.forEach((prop, mapper, codec) => {
            if (!mapper.encoded) {
                return;
            }

            const value = object && object[prop];
            const valueType = new ValueType(mapper.type, codec);
            return valueType.encodeProperty(value, providers)
            .then(value => {
                result[mapper.mapped] = value;
            });
        })
        .then(() => result);
    }
    decode(json, providers) {
        ...
        return this.mapping.forEach((prop, mapper, codec) => {
            ...
            const value = json && json[mapper.mapped];
            const valueType = new ValueType(mapper.type, codec);
...
module.exports.stringCodec = Codec.custom({
    encode: value => value === undefined || value === null ? value : String(value),
    decode: value => value === undefined || value === null ? value : String(value)
});

module.exports.numberCodec = Codec.custom({
    encode: value => value === undefined || value === null || Number.isNaN(Number(value)) ? value : Number(value),
    decode: value => value === undefined || value === null || Number.isNaN(Number(value)) ? value : Number(value)
});

module.exports.booleanCodec = Codec.custom({
    encode: value => value === undefined || value === null ? value : Boolean(value),
    decode: value => value === undefined || value === null ? value : Boolean(value)
});
